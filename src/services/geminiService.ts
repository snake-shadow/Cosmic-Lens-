...
const generateProceduralData = (name: string): CelestialData => {
  ...
  // Basic heuristic to make procedural data slightly less random
  let typeIndex = seed % CELESTIAL_TYPES.length;
  if (lowerName.includes('comet') || lowerName.includes('atlas')) typeIndex = 999; // Special case for comet
  if (lowerName.includes('star') || lowerName.includes('sun')) typeIndex = 2; // Supergiant
  if (lowerName.includes('hole')) typeIndex = 4; // Black Hole
  
  let type = CELESTIAL_TYPES[typeIndex] || CELESTIAL_TYPES[seed % CELESTIAL_TYPES.length];
  if (typeIndex === 999) type = "Comet";
